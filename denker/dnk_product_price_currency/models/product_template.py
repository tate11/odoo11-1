...
class ProductTemplate(models.Model):
    ...
    dnk_price_currency_id = fields.Many2one('res.currency', string='- Price List Currency', default=_default_currency)
    ...
    def price_compute(self, price_type, uom=False, currency=False, company=False):
        ...
        if price_type in ['list_price', 'list_price']:  # pretul pubilc poate fi in euro !
            for template in self:
                ...
                prices[template.id] = template.dnk_price_currency_id.compute(prices[template.id],
